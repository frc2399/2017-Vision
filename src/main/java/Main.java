...
//TODO make idouble boilerHieght equivilant to actual height of boiler when finished
double heightOfBoilerTape = 39.5; //this is from the top of the 2 in boiler retroreflective tape to the floor and in inches
...
//	 Imgproc.cvtColor(inputImage, rgb, Imgproc.COLOR_BGR2RGB);
      Core.inRange(rgb, new Scalar(rgbThresholdRed[0], rgbThresholdGreen[0], rgbThresholdBlue[0]), new Scalar(rgbThresholdRed[1], rgbThresholdGreen[1], rgbThresholdBlue[1]), rgb);
...
//	System.out.println("unfiltered contour number " + i + "'s area is " + area);
//	int contourNumber;
...
/*	if(Imgproc.arcLength(new MatOfPoint2f(contour.toArray()), true) < minPerimeter){
	...
	else if (solid < solidity[0] || solid < solidity[1]){
		break;
	}

	else if((contour.rows() < minVertices) || (contour.rows() > maxVertices)){
		break;
	}
...
/*      if(!contours1.isEmpty())
	{
	   Rect r = Imgproc.boundingRect(contours1.get(0));
	   centerX = r.x + (r.width/2);
      Imgproc.rectangle(rgb, new Point(r.x, r.y), new Point(r.width, r.height), new Scalar(225, 0, 0));
      smartDash.putNumber("Rectangle Height", r.height);
      smartDash.putNumber("Rectangle Width", r.width);

	}

      smartDash.putNumber("Center X", centerX);
      smartDash.putString("DB/String 0", "TEST");
*/
